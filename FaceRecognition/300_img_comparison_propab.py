...
for filename in os.listdir(folder_path):
    if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
        ...
        if face_locations:
            ...
            # Рассчитываем вероятность (размер лица относительно изображения)
            face_area = (bottom - top) * (right - left)
            img_area = img.shape[0] * img.shape[1]
            face_confidence = round((face_area / img_area) * 100, 2)  # В процентах
            
            cv2.rectangle(img, (left, top), (right, bottom), (0, 255, 0), 2)
            print(f"✅ Лицо найдено: {filename} (вероятность: {face_confidence}%)")
        else:
